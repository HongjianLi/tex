...
On both PDBbind v2007 (Table \ref{tbl:tst195}) and v2012 (Table \ref{tbl:tst201}) benchmarks, MLR delivered the best performance when it was trained with the 247 carefully selected complexes. Its performance dropped when more complexes were used for training. On PDBbind v2013 blind benchmark (Table \ref{tbl:tst382}), MLR delivered the best performance when it was trained with 792 complexes rather than the fewest 247 complexes or the most 2897 complexes, although it displayed approximately the same performance when using training samples of five different volumes.
...
When the features are few (x=4), RF performance does not necessarily scale up with more training samples.

x=46 and trn=2280 best over x={4,10,40,46} and trn={247,2280}. More x and trn useful for RF. True on both tst195 and tst201.
tst382: More trn leads to better prediction for all x=4,10,42,46. not only the difference in performance is substantial but grows as more data is available for training, thus increasing the importance of using RF. tst382 can help to see the trend more clearly. Machine-learning scoring functions assimilate data better than empirical scoring functions

more training samples, more knowledge to learn.
...
RF in contrast can exploit them. sufficiently capture the non-linearity. overtrain, generalization error.
...
More features and training samples boost RF performance but not MLR. Improvements with training dataset size can only be gained with the appropriate regression model anyway. given tremendous abundance of training samples.

MLR requires  carefully devised and usually sophisticated features, but RF can also benefit from simple ones.

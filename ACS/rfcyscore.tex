...
On PDBbind v2007 core set (N=195) (Table \ref{tbl:tst195}), 1) MLR, trn=247 best. 2) RF, trn1105 best. Every complex in the test set has complexes involving the same protein in the training set. The superior performance of RF-Score was highlighted by, who nevertheless attribute it to the characteristics of the most widely-used benchmark \cite{774}. Conclusion: refined07 minus core07 not a good benchmark.

On PDBbind v2012 core set (N=201) (Table \ref{tbl:tst201}), 1) MLR, trn=247 best.

On PDBbind v2013\textbackslash v2012 refined set (N=382) (Table \ref{tbl:tst382}), 1) trn=792 best.
...
x=4, 1) m=RF, trn1105>trn247>trn2280

tst=382 Table \ref{tbl:tst382}
m=MLR, trn792>trn2897>trn2059>trn1300. Using fewer training samples is slightly better. Do not scale up, almost same performance for trn=792,1300,2059,2897.
m=RF, 1) More trn leads to better prediction for all x=4,10,42,46. 2) x=4, RF(trn2280)~MLR(trn792). x=10,42,46, RF>MLR for any trn.

RF in contrast can exploit them.

2) m=RF , x=46 and trn=2280 best over x={4,10,40,46} and trn={247,2280}. More x and trn useful for RF.
...
More features and training samples boost RF performance but not MLR
...
improvements with training dataset size can only be gained with the appropriate regression model anyway.

...
idock introduces a novel feature that can automatically detect and deactivate inactive torsions, which are presented and activated in the input file but indeed have no impact on the overall scoring, such as \textemdash{OH}, \textemdash{NH$_2$} or \textemdash{CH$_3$}, which only rotate the hydrogens and thus have no contributions to the idock score. idock is capable of re-classifying them as inactive torsions during parsing, thus reducing the dimension of variables to optimize in subsequent BFGS iterations.

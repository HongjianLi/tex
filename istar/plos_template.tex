...
\subsection*{Web Site}
On 17 December 2012, W3C (Worldwide Web Consortium) finalized the specification of HTML5. This means developers have a stable target to write applications for. Compared with HTML4, HTML5 provides a way to write rich and real-time web applications. We have thus transformed the famous Twitter Bootstrap template into our HTML5-powered istar web site.
...
The second key feature of istar is the support of monitoring job progress in real time. Thanks to the real-time features defined in HTML5, we have managed to compose a timer to automatically fetch and report the latest job progress every second in an Ajax (Asynchronous JavaScript and XML) fashion. Users can have a rough estimation in advance of how long the jobs will take and when the jobs will complete. This feature is particularly handy when the jobs are long running, which are the cases of large-scale virtual screening.

\subsection*{Daemon}
After jobs are submitted and queued, it is the daemon that indeed performs protein-ligand docking. A daemon is a program running in the background silently and doing some kind of jobs. We have derived a customized daemon from the offline version of idock, making the daemon to iteratively fetch a pending job, reuse the receptor structure and grid maps, perform 2-phase docking, exploit 2-level parallelism, report job progress in real time, write verbose output to file, and send email notification upon job completion.

Since the initial release of idock 1.0 in 2011, we have revamped it to version 2.1, further improving docking speed and accuracy, inventing new functionalities, and fixing bugs. Please refer to the change log on the idock web site for a complete list of enhancements and bugfixes. Here we highlight some key features briefly. idock implements a lightweight thread pool to reuse threads and maintain a high CPU utilization throughout the entire screening procedure. idock implements a lightweight thread-safe progress bar to report progress in real time. idock automatically resumes docking from the previous stopping point in case the process gets killed accidentally. idock supports reading and writing compressed ligand files in gzip/bzip2 format. We have tried every endeavor to optimize idock to run faster, making it especially appropriate for large-scale docking.
...
\subsection*{Comparison of Online and Offline idock}
Since there is an offline version of idock, it is natural to ask why not run idock locally on one's computer instead of submitting jobs to istar? The major reason is automation.

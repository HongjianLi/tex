...
idock introduces a novel feature that can automatically detect and deactivate inactive torsions, which are presented and activated in the input file but indeed have no impact on the overall scoring, such as hydroxyl \textemdash{OH}, amine \textemdash{NH$_2$} or methyl \textemdash{CH$_3$}, which only rotate the hydrogens and thus have no contributions to the idock score. idock is capable of re-classifying them as inactive torsions during parsing, thus reducing the dimension of variables to optimize in subsequent BFGS iterations.
...
\end{center}
\caption{
{\bf Boxplots of the number of rotatable bonds (NRB) against the successful and unsuccessful cases of idock and AutoDock Vina ($N$ = 201).} Very often the $RMSD$ of 2.0\AA\ is regarded as the positive control for correct bound structure prediction.
}
\label{Program-NRB}
\end{figure}

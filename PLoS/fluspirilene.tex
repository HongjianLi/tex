...
44 X-ray crystallographic structures of CDK2 in complex with a bound ligand (Table \ref{PDBs}) were collected from the PDB (Protein Data Bank) \cite{540,537}. A previously written script \cite{1362} was re-used to automatically define the docking search space by finding the smallest cubic box that covers the entire co-crystallized ligand and subsequently extending the box by 10\AA\ in all the three dimensions. The 44 CDK2 structures were manually extracted from their corresponding complexes with the co-crystallized ligands and waters removed, and then converted from PDB format to PDBQT format using the prepare\_receptor4.py script of AutoDockTools \cite{596}.
...
Table S2. Ensemble docking results of the 4,914 compounds. The table comprises the ZINC ID, the average score and the individual scores for the 44 selected CDK2 structures, and the molecular properties.%dbap+fda.csv

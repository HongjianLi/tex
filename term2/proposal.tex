...
We use AMD CodeAnalyst Performance Analyzer for hotspot detection. Figure ref{} shows thread profile.
...
Each SMX has 64 KB of on-chip memory that can be configured as 48 KB of shared memory with 16 KB of L1 cache, or as 16 KB of shared memory with 48 KB of L1 cache. Since threads within a thread block run their Monte Carlo tasks independently and rarely communicate with one another, we decide to allocate 48 KB of the 64 KB on-chip memory to L1 cache.

Maximizing instruction throughput can be achieved by using single precision instead of double precision and using intrinsics instead of regular functions. This suggests trading precision for speed when it does not affect the end result.

float vs double

\section{Real-Life Drug Discovery Case Studies}

Our colleges and collaborators at biochemistry and pharmacy departments 
Having developed and advertised idock, we kept receiving constant docking requirements from 

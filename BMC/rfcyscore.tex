...
\endminipage
\caption{Prediction performance of regression models MLR and RF trained with varying numbers of features and samples in terms of root mean square error RMSE (first row), standard deviation SD in linear correlation (second row), Pearson correlation coefficient Rp (third row) and Spearman correlation coefficient Rs (fourth row). Left column: PDBbind v2007 benchmark (N=195). Center column: PDBbind v2012 benchmark (N=201). Right column: PDBbind v2013 benchmark (N=592).}
\label{fig:stat}
\end{figure}

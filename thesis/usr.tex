...
the reason why a database is populated with multiple conformers of each flexible compound is to reduce the possibility of missing compounds with similar shape to the query. Three levels of parallelisms are implemented. We present a novel AVX implementation of sum of absolute differences in order to calculate the USR or USRCAT scores between two moment vectors. Diverse input formats and iview visualization.

Our implementation of USR and USRCAT on istar is advantageous over other similar web servers such as UFSRAT \citep{1436}, EDULISS \citep{1437}, and SwissTargetPrediction \citep{1408} in terms of functionality and usability. It supports SDF, MOL2, XYZ, PDB and PDBQT formats for the input query. This is an advantage over the UFSRAT \citep{1436} web server, where the input format of query ligand is limited to SDF only. Moreover, our daemon supports multiple query ligands in a single job.

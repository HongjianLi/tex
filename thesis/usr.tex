...
\begin{equation}
\mu_1=\frac{1}{n}\sum_{i=1}^{n}{d_i}
\label{usr:moment1}
\end{equation}

\begin{equation}
\mu_2=\sqrt[2]{\frac{1}{n}\sum_{i=1}^{n}{(d_i-\mu_1)^2}}
\label{usr:moment2}
\end{equation}

\begin{equation}
\mu_3=\sqrt[3]{\frac{1}{n}\sum_{i=1}^{n}{(d_i-\mu_1)^3}}
\label{usr:moment3}
\end{equation}
...
There are numerous 2D-to-3D conversion tools that can generate 3D molecular conformations from a considered 2D chemical structure, such as Cyndi \citep{1393,1394} and OMEGA \citep{462}. A study \citep{1127} examined the performance of four freely available small molecule conformer generation tools, Balloon \citep{1442}, Confab \citep{1443}, Frog2 \citep{1444}, and RDKit (http://www.rdkit.org/), alongside a commercial tool, MOE (http://www.chemcomp.com/), and found that RDKit and Confab were statistically better than other methods at generating low RMSD (Root Mean Square Deviation) conformers to the known structure, and RDKit resulted as the second fastest method after Frog2. The study also developed a postprocessing algorithm to build a diverse and representative set of conformers to enhance the performance of RDKit. Therefore to generate conformers in this study we will use RDKit in combination with the postprocessing algorithm.% Also mention the C++ version rather than Python. RDKit has the constraint of not generating new conformers that have a large RMSD to existing conformers, so postprocessing is required, as suggested in \citep{1127}.

...
%The five scaling factors are ow (all atoms), hw (hydrophobes), rw (aromatic atoms), aw (acceptors) and dw (donors).

\begin{eqnarray}
S(\mathbf M^i, \mathbf M^j, ow, hw, rw, aw, dw)=(1
&+&ow\frac{1}{12}\sum_{k= 1}^{12}|\mathbf M_k^i-\mathbf M_k^j|+hw\frac{1}{12}\sum_{k=13}^{24}|\mathbf M_k^i-\mathbf M_k^j|
&+&rw\frac{1}{12}\sum_{k=25}^{36}|\mathbf M_k^i-\mathbf M_k^j|+aw\frac{1}{12}\sum_{k=37}^{48}|\mathbf M_k^i-\mathbf M_k^j|
&+&dw\frac{1}{12}\sum_{k=49}^{60}|\mathbf M_k^i-\mathbf M_k^j|)^{-1}\in[0, 1]

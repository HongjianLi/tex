...
\begin{equation}
t=(t_r\cdot m_{conf}+t_s\cdot m_{conf}\cdot n_{query})\cdot(n_{conf}/m_{conf})\cdot n_{job}=t_r\cdot n_{conf}\cdot n_{job}+t_s\cdot n_{conf}\cdot n_{query}\cdot n_{job}
\label{usr:time1}
\end{equation}

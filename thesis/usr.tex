...
Our client side UI (User Interface) supports SDF, MOL2, XYZ, PDB and PDBQT formats for the input query. This is an advantage over the UFSRAT \citep{1436} web server, where the input format of query ligand is limited to SDF only. Moreover, our daemon supports multiple query ligands in a single job, another advantage over the UFSRAT \citep{1436} and the EDULISS \citep{1437} web servers.

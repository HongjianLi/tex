...
To enable fast screening, it is possible to preload all descriptors into memory given a server with sufficient capacity of memory. Mathematically, suppose $t_r$ is the reading time of a moment vector of a conformer in the database, $t_s$ is the scoring time of two moment vectors, $n_{conf}$ is the number of conformers in the database, $n_{query}$ is the number of query ligands of a single job, and $n_{job}$ is the number of jobs, the total screening time $t$ would be
...
is prohibitive on computers nowadays. So have to be chunk by chunk. Suppose in a job there are n conformers but only c<<n fit in the memory, and there are $q$ query ligands. The way to do this using a single core is to loop n/c times, each time preloading a different chunk with c conformers and then making the q queries on this chunk storing all IDs above the similarity threshold. After the loop, hits are merged from each chunk. total time $t=t_r+q\cdot t_s$ but if you preload all for each query $t=q(t_r+t_s)$ with $t_r$ (reading descriptors time) and $t_s$ (screening conformers time).
...
AVX (Advanced Vector Extensions) instructions, 256 bit. AVX2 512 bit. Use PowerPoint to draw a diagram here to illustrate. unrolling, load, sub, abs (andnot), hadd (horizontal add). Moments are in double type, 64bit, so each time can process four elements in the moment vector. Unrolling results in a longer sequence of instructions but faster execution due to the circumvention of the use of a loop iterator. Unrolling times depend on qn=12 or 60, USR or USRCAT.

\begin{figure}
\begin{center}
\includegraphics[width=\linewidth]{../usr/AVX.png}
\end{center}
\caption{AVX instructions used to compute USR/USRCAT scores.}
\label{usr:AVX}
\end{figure}
...
the reason why a database is populated with multiple conformers of each flexible compound is to reduce the possibility of missing compounds with similar shape to the query. Three levels of parallelisms are implemented. AVX instructions. Diverse input formats and iview visualization.

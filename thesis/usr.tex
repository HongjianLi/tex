...
The database contains 23,129,083 ligands, on average each has 10 conformers, each molecule's moment vector has 60 elements of double type (64bit, or 8bytes) so its moment vector consists of 8*60*10*23,129,083=111,019,598,400B=103GB. Preloading all descriptors into memory is prohibitive on computers nowadays. So have to be chunk by chunk. Suppose in a job there are n conformers but only c<<n fit in the memory, and there are $q$ query ligands. The way to do this using a single core is to loop n/c times, each time preloading a different chunk with c conformers and then making the q queries on this chunk storing all IDs above the similarity threshold. After the loop, hits are merged from each chunk. total time $t=t_r+q\cdot t_s$ but if you preload all for each query $t=q(t_r+t_s)$ with $t_r$ (reading descriptors time) and $t_s$ (screening conformers time).
